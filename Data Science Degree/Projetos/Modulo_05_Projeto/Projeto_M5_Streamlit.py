...
st.sidebar.subheader('modulo_5_projeto - História das Olimpíadas')
...
if sb == 'Questão 2':
    st.subheader(
        '2. Será que existe alguma correlação entre a altura do atleta e seu desempenho, para algum esporte? Faça essa'
        'análise separadamente para cada gênero, e conclua.')
    st.markdown('<hr>', unsafe_allow_html=True)

    # apresentação da análise

    '''
	Para a seguinte análise, foi decidido definir desempenho como o simples fato do atleta ter conquistado uma medalha,
	independente do seu valor, então, além de Não-Medalhista, teremos todos os valores de Bronze, Silver e Bronze sendo
	convertidos para Medalhista, como podemos ver abaixo.
	'''
    ...
    '''
	Foram removidas as duplicatas, usando o metodo drop_duplicates, de modo a manter a primeira ocorrência da modalidade
	(Event) por ano (Year).
	'''
    ...
    '''
	Abaixo segue uma análise geral de correlação entre altura e desempenho utilizando-se um mapa de calor. Para tal,
	pegamos os dummies de altura e medalha, obtendo o mapa abaixo.
	'''
...
if sb == 'Questão 3':
    ...
    '''
	Para essa questão primeiro se utilizou o banco binário (medalhista e não-medalhista) para determinar quantas
	medalhas cada esporte trouxe para casa, com o cuidado de remover a duplicatas.
	'''
    ...
    '''
	Então organizou-se este banco de maneira agrupada após filtrar de maneira que contivesse apenas os 6 esportes com
	mais medalhas, identificados acima.
	'''
    ...
    '''
	No gráfico abaixo, um histograma de barras normalizadas em 100%, podemos observar a proporção de medalhas trazidas
	para casa, bem como a base, isto é, o valor total de medalhas conquistadas no topo.
	'''
...
if sb == 'Questão 4':
    ...
    '''
	No gráfico abaixo podemos observar um histograma de barras com as quantidades de medalhas obtidas a cada 4 anos,
	proximadamente. Esse gráfico se mostra útil, por em apenas uma visualização ser capaz de informar não apenas a
	quantidade de melhadas por tipo, como também observar o aumento das vitórias ao longo dos anos.
...
if sb == 'Questão 5':
	...
	2. Top5 de Países para cada um destes esportes: determinar quais são os países que conquistaram mais medalhas nestes
	mesmos esportes identificados no passo 1.
	Objetivo final: verificar se o Brazil está entre os 5 países top, nos mesmos esportes em que tem melhor desempenho.
	E se sim, em que posição se encontra para cada um.
	'''
    ...
    '''
	Conclusão: O Brasil ainda se coloca entre os Top 5 com 3 esportes, com boas pontuações, ficando em 5o lugar no
	Veleiro (?), 2o lugar em Volley e 1o Lugar no Futebol, como esperado. Vale pontuar que dois dos esportes que se tem
	mais medalhas no Brasil, não chegam a figurar entre os 5 melhores países no panorama mundial.
	'''
